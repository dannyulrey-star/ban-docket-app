...
5. Test it in your browser by visiting `http://localhost:3001/api/bans` — you should see
...
3. Set:
   ...
   - **Instance Type:** Free
4. Under **Environment Variables**, add `MONGODB_URI` with the same connection string
   from your local `.env`.
5. Click **Create Web Service**. Render will build and give you a live URL like
   `https://ban-docket-backend.onrender.com`.
6. Visit `https://ban-docket-backend.onrender.com/api/bans` to confirm it responds
...
Send that URL to your friends — that's it, no Claude, no login, nothing to install
on their end.
...
- Add a dropdown of your actual friend-group names instead of free-text.
- Add a vote/veto system before a ban is official.
- Add simple authentication so only your group can file bans.
